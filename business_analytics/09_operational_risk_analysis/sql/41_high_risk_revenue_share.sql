...
SELECT
  ...
  SUM(gross_revenue) FILTER (WHERE risk_bucket = 'HIGH RISK') AS high_risk_gross_revenue,
  (SUM(gross_revenue) FILTER (WHERE risk_bucket = 'HIGH RISK'))::numeric
    / NULLIF(SUM(gross_revenue), 0) AS high_risk_revenue_share
FROM bucketed;
